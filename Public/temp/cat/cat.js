...
//保存@bname@分类信息
function save@bname@Cat(index) {
    $.post(c_path + '/save@bname@Cat', {"form_data": $('#form_data').serialize()}, function (result) {
        if (result.code == '500') {
            layer.close(index);
...
function delCatLayer(id) {
    layer.confirm('确定要删除此信息嘛？', {
        ...
        $.post(c_path + '/del@bname@Cat', {'id': id}, function (result) {
            if (result.code == '500') {
                layer.msg(constants.SUCCESS, {time: 1000}, function () {
                    location.reload();
                });
...
function createLink(id) {
    ...
    layer.alert('该栏目链接为：</br>'+webroot + '/index.php/home/@sname@/@sname@_list?cat_id=' + id);
}
