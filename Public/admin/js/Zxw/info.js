...
$(function () {
    ...
    $('#saveInfo-btn').click(function () {
        $.post(c_path + '/saveZxwInfo', {"form_data": $('#save-form').serialize()}, function (result) {
            if (result.code == '500') {
...
function delInfoLayer(id) {
    layer.confirm('确定要删除此信息嘛？', {
        ...
        $.post(c_path + '/delZxwInfo', {'id': id}, function (result) {
            if (result.code == '500') {
                layer.msg(constants.SUCCESS, {time: 1000}, function () {
                    location.reload();
                });
...
function pubInfoLayer(id) {
    layer.confirm('确定要发布此信息嘛？', {
        ...
        $.post(c_path + '/publZxwInfo', {'id': id}, function (result) {
            if (result.code == '500') {
                layer.msg(constants.SUCCESS, {time: 1000}, function () {
                    location.reload();
                });
