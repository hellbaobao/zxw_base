...
class ZxwCatController extends BaseDBController {
    ...
    public function saveZxwCat() {
        ...
        if ($returnData['code'] == '500') {
            ...
            $logC = A('Actionlog')->addLog('ZxwCat', 'saveZxwCat', '添加/编辑Zxw分类');
        }
    ...
    /**
     * function:删除Zxw分类及分类下的Zxw信息
     */
    public function delZxwCat() {
        $condition['id'] = array('EQ', $_POST['id']);
        ...
        if (count($childCatArray) > 0) {
            foreach ($childCatArray as $value) {
                $zxwInfoList = $this->infoModel->where(array('cat_id' => $value['id']))->select();
            }
        }
        if (false !== $this->catModel->where($condition)->delete()) {
            ...
            if (count($zxwInfoList) > 0) {
                foreach ($zxwInfoList as $value) {
                    $this->catModel->where(array('id' => $value['id']))->delete();
                }
            }
            $returnData['code'] = '500';
            $logC = A('Actionlog')->addLog('ZxwCat', 'delZxwCat', '删除Zxw分类');
        } else {
            $returnData['code'] = '502';
        }
        $this->ajaxReturn($returnData, 'JSON');
    ...
    public function delArrayCat() {
        ...
        $logC = A('Actionlog')->addLog('ZxwCat', 'delArrayCat', '删除Zxw分类');
    }
